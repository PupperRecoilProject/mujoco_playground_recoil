...
# ==============================================================================
# START: CLEANED UP CODE FOR MODIFIED SAC
# ==============================================================================

def wrap_for_sac_training(
    env: mjx_env.MjxEnv,
    episode_length: int = 1000,
    action_repeat: int = 1,
    # **kwargs 用來接收並忽略 sac.train 可能傳入的 randomization_fn
    **kwargs,
) -> Wrapper:
  """
  Applies the necessary wrappers for the MODIFIED Brax SAC training.

  Since our modified `sac.train` now natively handles dictionary observations
  and its training loop manages resets, the wrapping chain is simpler than PPO's.
  We primarily need to handle vectorization (Vmap) and episode length.

  Args:
    env: The MuJoCo Playground environment to be wrapped.
    episode_length: The maximum length of an episode.
    action_repeat: The number of times an action is repeated for each step.
    **kwargs: Accepts and ignores additional arguments.

  Returns:
    A wrapped environment ready for the modified SAC training.
  """
  # 步驟 1: 應用 VmapWrapper 來處理並行環境。
  # 這是必須的，因為 sac.train 使用 pmap 並傳遞批量的 key。
  env = brax_training.VmapWrapper(env)
  
  # 步驟 2: 應用 EpisodeWrapper 來處理回合長度和 action_repeat。
  env = brax_training.EpisodeWrapper(env, episode_length, action_repeat)
  
  # 【結論】: 我們不再需要任何自定義的包裝器了！
  # SacObservationWrapper -> 不需要，因為 sac.train 現在能處理字典。
  # PatchedEpisodeWrapper -> 不需要，因為 sac.train 現在能處理字典，
  #                        EpisodeWrapper 內部的 scan 不會再遇到 Pytree 衝突。
  
  return env

# ==============================================================================
# END: CLEANED UP CODE FOR MODIFIED SAC
# ==============================================================================